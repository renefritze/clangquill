...
| `group_by` | `clangquill_group_by` | `"symbol"` | How to partition output pages: `"symbol"` (one page per top-level symbol), `"file"` (one page per parsed source file), or `"class"` (split each namespace/class container into a page per documented child class/namespace). |
